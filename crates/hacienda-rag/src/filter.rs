...
//!
//! Recovered from `77e2fd3d71^:crates/xberg-rag/src/filter.rs` (xberg, MIT).
//! A small, backend-neutral predicate language over document- and chunk-level
//! fields, with hard complexity caps so a malicious or accidental filter
//! cannot blow up a backend.
...
mod tests {
    ...
    #[test]
    fn should_parse_field_when_doc_prefix_is_whitelisted() {
    ...
    #[test]
    fn should_parse_field_when_metadata_path_is_nested() {
        let parsed = FilterField("doc.metadata.foo.bar".to_string())
            .parse()
            .unwrap();
    ...
    #[test]
    fn should_reject_field_when_chunk_field_is_unknown() {
        assert!(FilterField("chunk.arbitrary".to_string()).parse().is_err());
    }

    #[test]
    fn should_reject_field_when_shaped_like_sql_injection() {
        assert!(FilterField("chunk.id; DROP TABLE chunks--".to_string())
            .parse()
            .is_err());
    }

    #[test]
    fn should_reject_field_when_prefix_is_invalid() {
        assert!(FilterField("unknown.field".to_string()).parse().is_err());
    }

    #[test]
    fn should_validate_ok_when_filter_is_flat() {
    ...
    #[test]
    fn should_reject_validate_when_field_is_unknown() {
    ...
    #[test]
    fn should_reject_validate_when_node_count_exceeds_cap() {
        let filters = (0..65)
            .map(|i| Filter::Eq {
                field: FilterField("doc.title".to_string()),
    ...
    #[test]
    fn should_reject_validate_when_text_match_query_is_oversize() {
    ...
    #[test]
    fn should_round_trip_when_filter_is_serialized_through_json() {
