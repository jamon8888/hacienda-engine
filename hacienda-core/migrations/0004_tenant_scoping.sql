...
--
-- That condition does NOT hold for every table below. Only `AuditStore` was actually
-- updated to write `tenant_id` explicitly in this change (S1b Task 2) --
-- `ReviewStore`/`JobStore`/`DocumentVersionStore`/`PresetStore`/`ApiKeyStore` remain
-- unimplemented (Tasks 3-6). Dropping the default on their columns here would make every
-- existing, un-updated `INSERT` on those tables fail its `NOT NULL` constraint the moment
-- this migration lands -- caught by `postgres-store-tests` failing on exactly that
...
-- since this PR only finishes the Rust side for one of the six tables.
--
...
-- The five tables below keep tenant_id's default until their own store gets the S1b
-- Task 3-6 treatment -- see the file-level comment above for why dropping it now would
