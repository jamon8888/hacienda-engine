...
use std::sync::Arc;
use xberg::{
    plugins::{register_post_processor, PostProcessor, ProcessingStage},
    ExtractedDocument, ExtractionConfig, Result, XbergError,
};
...
impl xberg::Plugin for HaciendaPiiProcessor {
    ...
    fn description(&self) -> &str {
        "Hacienda PII detection & redaction"
    }
    fn author(&self) -> &str {
        "hacienda team"
    }
...
impl PostProcessor for HaciendaPiiProcessor {
    ...
    fn priority(&self) -> i32 {
        60
    } // After built-in redaction (50)

    fn should_process(&self, doc: &ExtractedDocument, config: &ExtractionConfig) -> bool {
        doc.content.chars().any(|c: char| c.is_ascii_alphanumeric()) && config.redaction.is_some()
    }
    ...
    async fn process(&self, doc: &mut ExtractedDocument, config: &ExtractionConfig) -> Result<()> {
        ...
        let entities: Vec<xberg::types::Entity> = result
            .entities
            .iter()
            .map(|e| xberg::types::Entity {
