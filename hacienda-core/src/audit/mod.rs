...
pub use sink::{FileSink, AuditSink};
pub use exporter::{AuditExporter, ExportFormat};
pub use verifier::verify_chain;

use pii_audit::{AuditChain as PiiAuditChain, FileSink as PiiFileSink, AuditEntry as PiiAuditEntry, AuditSink};
...
impl AuditChainWrapper {
    pub fn new(config_hash: String) -> Self {
        Self { inner: PiiAuditChain::new(config_hash) }
    }

    pub fn append(&mut self, entry: AuditEntry) -> Result<(), String> {
        self.inner.append(entry.into())
...
impl FileSinkWrapper {
    pub fn new(path: impl AsRef<std::path::Path>, max_size: u64, max_files: u32, config_hash: String) -> Result<Self, String> {
        Ok(Self {
