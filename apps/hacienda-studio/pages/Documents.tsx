...
export function Documents({
  ...
  function handleDeleteFile() {
    ...
    toast.success(`« ${selectedFileDoc.result.frontmatter.source} » supprimé`);
    setSelected(null);
  }
  function handleDeleteFolder() {
    ...
    const folderName = selected.path.replace(/\/$/, "").split("/").pop() ?? "dossier";
    toast.success(`« ${folderName} » supprimé — ${names.length} fichier${names.length === 1 ? "" : "s"}`);
    setSelected(null);
  }

  return (
    <div className="flex flex-1 flex-col bg-background px-4 py-6 sm:px-6">
      {/* Centered page shell */}
      <div className="mx-auto flex w-full max-w-[1100px] flex-1 flex-col">
        <div className="mb-4 flex flex-col gap-3">
          <div className="flex flex-col gap-3 lg:grid lg:grid-cols-[1fr_auto_1fr] lg:items-center">
            ...
            <div className="flex justify-start lg:justify-center">
              {selected ? (
                <div className="inline-flex max-w-full items-center gap-2 rounded-full border border-border bg-card px-2 py-1.5 shadow-md">
                  <span className="hidden size-7 items-center justify-center rounded-full bg-muted text-muted-foreground sm:flex">
                    {selected.kind === "folder" ? <Folder className="size-3.5" /> : <FileText className="size-3.5" />}
                  </span>
                  <span className="max-w-[18ch] truncate font-mono text-xs font-medium text-foreground/90 sm:max-w-[24ch]">
                    {selected.kind === "folder"
                      ? (selected.path.replace(/\/$/, "").split("/").pop() ?? "dossier") + "/"
                  ...
                  {selected.kind === "file" && selectedFileDoc ? (
                    <>
                      ...
                      <button
                        ...
                        className="flex size-7 items-center justify-center rounded-full bg-muted text-muted-foreground hover:bg-destructive/15 hover:text-destructive"
                    ...
                    <>
                      <Button
                        size="sm"
                        className="h-7 rounded-full px-3.5 text-xs disabled:opacity-40"
                      ...
                      <button
                        ...
                        className="flex size-7 items-center justify-center rounded-full bg-muted text-muted-foreground hover:bg-destructive/15 hover:text-destructive"
                        title={`Supprimer le dossier et ${selectedFolderDocs.length} fichier(s)`}
                      >
                        <Trash2 className="size-3.5" />
                      </button>
                    </>
                  ...
                  <button
                    ...
                    className="ml-1 flex size-6 items-center justify-center rounded-full text-muted-foreground hover:bg-muted hover:text-foreground"
                  >
                    ×
                  </button>
                </div>
               ) : (
                 <span className="hidden text-xs text-muted-foreground/60 lg:inline" aria-hidden>
            ...
            <div className="flex items-center gap-2 lg:justify-end">
              <Button variant="outline" size="sm" onClick={onAddFiles} className="h-8">
        ...
        <div className="flex min-h-[520px] flex-1 flex-col">
          {documents.length === 0 ? (
            <div className="grid flex-1 place-items-center rounded-xl border border-border bg-card p-10 text-center shadow-md">
              <div className="max-w-sm">
                <div className="mx-auto flex size-10 items-center justify-center rounded-xl border border-border bg-muted">
        ...
        <p className="mx-auto mt-3 max-w-[640px] text-center text-[11px] leading-relaxed text-muted-foreground/70">
