// Classic shadcn/ui recipe (@radix-ui/react-dialog), not ported from hacienda-private's
// own dialog.tsx (built on @base-ui/react's Dialog primitive) — see
...
const DialogContent = React.forwardRef<
  ...
  React.ComponentPropsWithoutRef<typeof DialogPrimitive.Content> & {
    /** Defaults to `true` (this component's original, only behavior). Set `false` when
     * the caller places its own close control instead — e.g. `file-system.tsx`'s viewer
     * dialog puts a `DialogClose` in its own toolbar for non-image previews, and a
     * second built-in X would double up. */
