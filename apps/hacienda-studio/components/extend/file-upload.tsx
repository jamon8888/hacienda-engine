...
type FileUploadProps = {
  ...
  // Consumers that already run their own validation/rejection UX (e.g. a
  // skip-notice summarizing junk + unsupported files) need the raw file list
  // — `matchesAccept` would silently drop files before they ever reach that
  // logic. `accept` still sets the DOM `accept` attribute (native OS picker
  // filtering) even when this is false; only the JS-side filter is skipped.
  filterAccept?: boolean
  id?: string
...
export function FileUpload({
  ...
  const commitFiles = React.useCallback(
    (nextFiles: FileList | File[]) => {
      const scopedFiles = Array.from(nextFiles).slice(0, multiple ? undefined : 1)
      const acceptedFiles = filterAccept
        ? scopedFiles.filter((file) => matchesAccept(file, accept))
        : scopedFiles
    ...
    [accept, filterAccept, multiple, onFilesAccepted, onFilesChange]
  )
