...
import { fileRelativePath, partitionFiles, formatSkippedMessage } from './lib/file-ingest';
...
// Track I1: a folder ingest can be large enough (sequential, one 614MB-model
// worker) that a user wants out partway through. Drives the Cancel button and
// is cleared on `batch-complete` (cancelled or not).
...
function handleWorkerMessage(event: MessageEvent) {
	...
	switch (type) {
		...
		case 'file-complete':
			...
			// Keyed by `frontmatter.source` (the input's relativePath), matching the
			// key `progress` messages use — `data.name` is the *output* filename
			// (extension swapped to `.md`), which never matches what `progress`
			// case above set the map's key to.
			progress = new Map(progress).set(data.frontmatter.source, { ...data, stage: 'complete', percent: 100 });
			break;
		case 'batch-complete':
			processing = false;
...
async function handleFiles(fileList: FileList | FileList | File[]): Promise<void> {
	...
	// Track I1: partition rather than filter-with-a-side-effecting `error`
	// overwrite — a folder drop mixes real documents with `.DS_Store`,
	// thumbnails, etc., and the old loop only ever showed the *last*
	// skipped file's reason.
	const { valid: validFiles, skipped } = partitionFiles(fileArray, validateFile);
	error = formatSkippedMessage(skipped);
	...
	// Send to worker
	const fileInputs: FileInput[] = await Promise.all(validFiles.map(async f => ({
		name: f.name,
...
<div class="app">
  {#if !onboardingComplete}
    <Onboarding
      {assets}
      {nerModelDegraded}
      onComplete={() => { onboardingComplete = true; localStorage.setItem('xberg-studio-visited', 'true'); }}
    />
  {:else}
    <header class="header">
    ...
    <main class="main">
      <section class="drop-zone" role="group" aria-label="Upload documents" ondrop={onDrop} ondragover={onDragOver}>
        <input
          ...
          accept=".pdf, .docx, .xlsx, .pptx, .odt, .ods, .odp, .eml, .msg, .pst, .png, .jpg, .jpeg, .gif, .webp, .tiff, .bmp, .svg, .srt, .vtt, .txt, .md, .json, .csv, .xml, .html, .mp3, .wav, .m4a, .ogg, .flac, .aac, .mp4, .mov, .webm, .mkv"
          class="file-input"
          onchange={(e) => { const files = (e.target as HTMLInputElement).files; if (files) handleFiles(files); }}
        ...
        <!-- Track I1: a separate input rather than toggling `webkitdirectory` on the
				     one above — browsers don't consistently support flipping that attribute
				     at runtime, and "pick files" vs "pick a folder" are genuinely different
				     user intents worth two affordances. -->
        <input
          type="file"
          id="folder-input"
          webkitdirectory
        ...
        />
        <label for="folder-input" class="folder-label">or select a folder</label>
      </section>

      {#if files.length > 0}
        <section class="progress-section" aria-live="polite">
          {#each files as file}
            {#if progress.has(fileRelativePath(file))}
              <ProgressBar file={{ name: fileRelativePath(file) }} update={progress.get(fileRelativePath(file))!} />
            {/if}
          {/each}
        </section>
        {#if processing}
          <button class="btn-cancel" onclick={cancelProcessing}>Cancel</button>
        {/if}
      {/if}
...
.folder-label { display: block; margin-top: var(--spacing); font-size: 0.85rem; color: var(--color-primary); cursor: pointer; text-decoration: underline; }
...
.btn-cancel { margin-top: var(--spacing); padding: var(--spacing) calc(var(--spacing) * 2); background: transparent; border: 1px solid var(--color-error); color: var(--color-error); border-radius: var(--radius); font-weight: 600; }
