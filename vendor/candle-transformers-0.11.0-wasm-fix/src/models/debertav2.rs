...
impl XSoftmax {
    pub fn apply(input: &Tensor, mask: &Tensor, dim: D, device: &Device) -> Result<Tensor> {
        ...
        // `where_cond`'s on-true/on-false operands must share a dtype (candle's cpu
        // backend dispatches `WCond::map` on `t`'s dtype and errors "dtype mismatch" if
        // `f` doesn't match, per `Map2`'s default binary-op dispatch) — hardcoding these
        // as f32 broke every non-f32 caller, in particular GLiNER2 loaded as F16 for
        // wasm32 (see vendor/candle-transformers-0.11.0-wasm-fix's crate-level README
        // for why this is vendored). Cast to `input`/`output`'s actual dtype instead of
        // assuming DTYPE (f32).
...
impl DebertaV2DisentangledSelfAttention {
    ...
    fn disentangled_attention_bias(
        ...
        // Hardcoded f32 here breaks every non-f32 caller once `broadcast_add`'d below
        // against `c2p_att`/`p2c_att`, which carry `query_layer`/`key_layer`'s actual
        // working dtype (f16 for a wasm32 GLiNER2 load) — same "dtype mismatch" class of
        // bug as `XSoftmax::apply` above. Match the model's working dtype instead of
        // assuming DTYPE (f32).
