...
# Get current key
CURRENT_KEY=$(vault kv get -field=active_key "secret/hacienda/$ENV/pseudonym" 2>/dev/null || echo "")
if [ -z "$CURRENT_KEY" ]; then
  echo "ERROR: No current key found for $ENV"
  exit 1
fi
...
vault kv put "secret/hacienda/$ENV/pseudonym/$ARCHIVE_KEY" key="$CURRENT_KEY"
...
# Update active key
if [ -n "$TENANT_ID" ]; then
  # For multi-tenant, we store per-tenant keys
  vault kv put "secret/hacienda/$ENV/pseudonym/tenant_$TENANT_ID" key="$NEW_KEY"
  echo "Updated tenant_$TENANT_ID key"
else
