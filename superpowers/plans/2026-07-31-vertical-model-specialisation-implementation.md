...
**Baseline:** to be recorded in Task 0 before any edit. `cargo clippy --all-targets --all-features -D warnings` and `cargo fmt --check` must be clean at plan start.
...
- [ ] Run `cargo test --workspace` and record the exact passed/failed/ignored counts in this file under **Baseline**.
- [ ] Run `cargo clippy --all-targets --all-features -D warnings` and `cargo fmt --check`; record clean or fix first.
- [ ] Run `grep -rn "^\[pii" --include=*.toml . ../hacienda-private` and record every config file that would be affected by a new `[pii]` key.
- [ ] Confirm `cargo check -p hacienda-core --features ner-candle` succeeds and record wall time (the host has ~3 GB RAM; if this thrashes, every model-touching task needs a different plan).
- [ ] **Capture the pre-change chain-hash literal Task 3 needs.** Run the existing `audit::entry` tests with a temporary `dbg!` (or a throwaway test) to print `AuditEntry::new(input("id-1"), "prev", 0).chain_hash` for the fixture in `entry.rs:192`, and paste the hex here. Task 3's backward-compatibility test is worthless without a literal captured *before* the field is added.
